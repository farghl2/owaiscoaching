...
export function PricingSection() {
  ...
  return (
    <section id="pricing" className="w-full flex flex-col gap-6 mt-6 lg:mt-0">
      ...
      {/* PRICING CARDS */}
      <MotionStagger className="flex flex-col md:flex-row flex-wrap justify-center gap-6 mt-6 items-stretch">
        {PLANS_MOCK.map((plan) => {
          ...
          return (
            <MotionStaggerItem key={plan.id} className="w-full md:max-w-sm">
              <motion.div
                whileHover={{
                  y: isPopular ? -12 : -8,
                  boxShadow: isPopular
              ...
              >
                ...
                <div className="flex flex-col items-center text-center gap-2 mb-4 pt-2">
                  ...
                  {plan.pricingOptions.length > 1 && (
                    <div className="w-full max-w-[220px] mt-2 mb-2">
                      <p className="text-xs text-gray-500 text-center mb-1.5 tracking-wider">{t('home.pricing.choose_period')}</p>
                      <select
